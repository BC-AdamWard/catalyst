 ...
 * Filters by categoryEntityId custom attribute
 * @param {number} categoryEntityId - Category entity ID (e.g., 23)
...
export const fetchVertexBrowseByCategory = cache(
  async (
    ...
    // Custom attributes use: attributes.key: ANY("value1", "value2")
    // Note: ANY() requires double quotes around values
    const categoryFilter = `(attributes.categoryEntityIds: ANY("${categoryEntityId}"))`;
      ...
      const response = await searchClient.search(
        {
          ...
          facetSpecs: [
            ...
            },
            {
              ...
              limit: 10,
              enableDynamicPosition: true,
              // @ts-expect-error - intervals is supported by the API but not in the SDK types
              intervals: [
...
export const fetchVertexBrowseByBrand = cache(
  async (
    ...
    // Custom attributes use: attributes.key: ANY("value1", "value2")
    // Note: ANY() requires double quotes around values
    const brandFilter = `(attributes.brandEntityIds: ANY("${brandEntityId}"))`;
    const combinedFilter =
