...
1. **Vertex Event Service** (`lib/vertex-retail/events.ts`)
   - ✅ **Node.js Runtime** - Uses official `@google-cloud/retail` SDK
...
1. Go to Google Cloud Console → Retail API
...
```
projects/YOUR_PROJECT/locations/global/catalogs/default_catalog/branches/0/products/123
```

The `formatProductId()` function in `events.ts` handles this automatically.
