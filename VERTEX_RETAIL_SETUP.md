...
GOOGLE_APPLICATION_CREDENTIALS=/Users/nathan.booker/Documents/GitHub/catalyst/core/vertex-retail-key.json
```

**Note:** The service account key file has been saved to `/core/vertex-retail-key.json` and is automatically ignored by git.
...
1. **Create or use an existing GCP project** (e.g., `bc-prototypes-njb`)

2. **Enable the Retail API**:
   ```bash
   gcloud services enable retail.googleapis.com --project=bc-prototypes-njb
   ```
...
4. **Grant necessary permissions**:
   ```bash
   gcloud projects add-iam-policy-binding bc-prototypes-njb \
     --member="serviceAccount:vertex-retail-search@bc-prototypes-njb.iam.gserviceaccount.com" \
...
5. **Create and download a key**:
   ...
   gcloud iam service-accounts keys create ~/vertex-retail-key.json \
     --iam-account=vertex-retail-search@bc-prototypes-njb.iam.gserviceaccount.com
   ```

6. **Copy the JSON content** from `~/vertex-retail-key.json` and paste it as a single-line string into the `GCP_SERVICE_ACCOUNT_KEY` environment variable.
